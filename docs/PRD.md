...
- R101b: Incremental asset discovery — discovery cost MUST be proportional to new/changed work, not library size. Our index stays the source of truth for processing `status` (Immich exposes no API to filter "assets lacking my marker" — tags/`asset_metadata` are writable but not searchable). Two mechanisms: a **fast-path** that skips the Immich scan when enough `pending` work is already queued, and a **lower-bound watermark delta scan** using `search/metadata` `updatedAfter` (advance the per-type watermark to max `updatedAt` only after a complete pass; idempotent reconcile covers boundary overlap and mid-scan inconsistency). `--full-scan` escape hatch; watermark reset on `schema_ver` bump; deletions out of scope. **Design**: `docs/superpowers/specs/2026-06-16-imp-018-incremental-discovery-design.md`.
